...
**94 tests across 13 files** as of this README (regenerated via `pytest --collect-only`, not hand-typed — see the file-by-file breakdown in [Project Structure](#project-structure) — on a container built `--no-cache` from `requirements.txt`).

**The exact current pass count is deliberately not stated as a single number here — it moved three times while this section was being written, and the honest reason why is itself worth documenting rather than papering over with a plausible-looking figure (see this project's own [Documentation Practices](#documentation-practices)).** In order: a first full run showed 92 passed / 2 failed (the 2 in `test_oauth_login.py` asserting "login rejected when no client ID is configured" — these fail specifically in an environment, like this one, that has real Google/Microsoft client IDs configured through Settings > OAuth Providers; the tests' own precondition no longer holds here, not an application bug). A second run showed 90/4 — two more failures in `test_students_at_risk.py`, traced to a real bug: `_preserve_app_state()` (the shared ingestion-test cleanup helper) deleted `analyze_jobs` rows before `upload_batches` rows that FK-reference them, so its `finally`-block cleanup itself threw a `ForeignKeyViolationError` once real leftover `upload_batches` rows existed. **Fixed** — the delete order in `_preserve_app_state()` now matches the FK dependency (`upload_batches` first). Investigating that surfaced something more serious: `backend/app/ml/ingested_capstone.csv` — the single on-disk file this whole project's real dataset lives in, with no archived fallback copy ever present in this environment — had been silently overwritten with a 2-subject/951-row test fixture by an interrupted ingestion test, degrading the **live dev server itself** (`GET /api/health` dropped to `dataset.rows: 950`, versus the real ~327,501). Root cause: `_isolate_ml_paths()` (the test helper whose own docstring claimed full isolation) never actually redirected `INGESTED_DATA_DIR` — the variable `main.py`'s real confirm handler writes through — so a real confirm() call inside a "isolated" test wrote straight to the same file the live server loads from. **Fixed**: `_isolate_ml_paths()` now redirects `INGESTED_DATA_DIR` (and actually reassigns `DATA_PATH`/`check_new_period.DATA_PATH`, which it had been capturing and restoring but never actually changing) to a throwaway temp directory. Also fixed the dev-server footgun that let this reach production data at all: `docker-compose.yml`'s uvicorn `--reload` watched the entire `backend/` tree including `tests/`, so editing a test file could trigger the live server to reload and re-read whatever was currently on disk. `--reload-dir` (even an absolute path) was tried first and, confirmed live, did **not** stop this — uvicorn 0.30.6 still watched `tests/` regardless (it appears to also implicitly track the cwd/app-module's own directory). What actually worked, verified live both directions (touching a `tests/` file → no reload; touching `app/main.py` → reload fires): `--reload-exclude "tests/*" --reload-exclude "scripts/*"`. **What's still open, honestly**: the fix stops this from recurring, but does not recover the corrupted data — no archived copy, Docker image, `PendingIngest` row, or OS-level backup (checked: Volume Shadow Copy needs elevation not available here; the project isn't under OneDrive sync) exists to restore it from. Re-uploading the real source file through Data Ingestion is the only path back to a clean count. Until then, the honest, current state of this suite includes real failures downstream of that missing data (`test_attendance_rate_known_input`, `test_retrain_trigger_same_period_vs_new_period`, and likely others under `test_smoke.py` that depend on real subject data) — confirmed to be pre-existing-data-dependent, not caused by the isolation fix, by re-running with the fix in place and observing the shared file stayed byte-unchanged.
