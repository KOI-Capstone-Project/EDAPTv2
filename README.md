...
- **A real, unresolved data-corruption incident was found while re-verifying this session's own changes, and is NOT yet fixed.** Running `test_ingestion_e2e.py`'s `test_retrain_trigger_same_period_vs_new_period` left the shared, live `backend/app/ml/ingested_capstone.csv` overwritten with a reduced (248,592-row, down from 327,501) and corrupted (one row with a student ID string in the `STUDYPERIOD` column) dataset — the same class of incident documented above in [Running Tests](#running-tests), reappearing. Root cause is not yet pinned down; the prime suspect is the new fire-and-forget `_launch_scoring_job` (see [Background risk-prediction scoring](#background-risk-prediction-scoring--prediction-caching)) leaving a scoring task pending past the point `_isolate_ml_paths()` tears down isolation, but this was not confirmed by direct reproduction before the investigation was paused — neither `_run_scoring_job` nor `_compute_roster_rows` were found to perform any file I/O themselves on inspection, so the actual mechanism remains unexplained, not just unfixed. **The live dev server's in-memory dataset was confirmed unaffected** (`GET /api/health` still reports 327,501 rows — a restart or scheduled retrain would be the point this becomes user-visible, since both read from disk), and a verified-clean export of it was taken as a safety net before this was handed back for a manual restore. **Do not treat the auto-after-ingest scoring trigger as fully verified safe under test until this is root-caused and fixed** — the manual "Run Risk Scoring Now" path and the caching logic itself are unaffected (verified separately via `test_roster_caching.py` and direct live checks), only the interaction between a fire-and-forget scoring task and this test file's isolation teardown is in question.
