...
**As of 2026-08-08: do not cite `0.8374` as this model's current fail-class recall without this caveat — see [Known Open Items](#known-open-items) and `model_card.md`'s Round 6 for the full, confirmed evidence (not a correlation: a 100% overlap between the 205 flipped enrolments and a traced resit-collapsing mechanism).** This session's data refresh corrected 205 real students' Fail label to Pass (zero flips the other direction), traced to richer resit/attempt history now present in the refreshed extract — not noise, not a bug in the resit-collapsing logic itself, which is confirmed correct on both the old and new data. Re-scoring this exact live model against the corrected labels alone drops recall to 0.8230; a model retrained on the corrected training data scores 0.7860. **`0.7860` is the more trustworthy current figure, not `0.8374`**, even though it's lower.
