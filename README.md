...
- **Automatic** — right after a capstone or attendance ingestion succeeds, a `ScoringJob` is enqueued (`trigger: "auto_after_ingest"`) for exactly the study periods that upload touched. Launched fire-and-forget via `_launch_scoring_job()` (`asyncio.create_task`, deliberately not `await`ed) rather than run sequentially inside the ingest job itself — a full institution-wide scoring pass can take from minutes to well over an hour (one real model+SHAP call per student), and awaiting it there would mean "Confirm and Ingest" stays stuck at "running" for that entire time, defeating the point of backgrounding ingestion in the first place. The `ScoringJob` row is created and pollable immediately, independent of the ingest job's own status. A bare `asyncio.create_task(...)` risks silent garbage collection once the enclosing function returns and nothing else references it — a real asyncio pitfall, not a hypothetical one — so the task is held in a module-level `_SCORING_BACKGROUND_TASKS` set, removed automatically via a done-callback once it finishes.
