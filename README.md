...
│   ├── tests/                          # 116 tests across 15 files (regenerated via pytest --collect-only) — see Running Tests
...
│   │   ├── test_scoring_job_control.py # 4 tests — cancel, serialized concurrency, orphaned-job self-heal
...
- **Automatic** — right after a capstone or attendance ingestion succeeds, a `ScoringJob` is enqueued (`trigger: "auto_after_ingest"`) for exactly the study periods that upload touched. Launched fire-and-forget via `_launch_scoring_job()` (`asyncio.create_task`, deliberately not `await`ed) rather than run sequentially inside the ingest job itself — a full institution-wide scoring pass can take from minutes to well over an hour, and awaiting it there would mean "Confirm and Ingest" stays stuck at "running" for that entire time, defeating the point of backgrounding ingestion in the first place. The `ScoringJob` row is created and pollable immediately, independent of the ingest job's own status. A bare `asyncio.create_task(...)` risks silent garbage collection once the enclosing function returns and nothing else references it — a real asyncio pitfall, not a hypothetical one — so the task is held in a module-level `_SCORING_BACKGROUND_TASKS` dict (keyed by job id, so a specific one can be found and cancelled later — see below), removed automatically via a done-callback once it finishes. The manual trigger uses the identical `_launch_scoring_job()` path (not a separate `BackgroundTasks.add_task` call it used to use), so every scoring job — auto or manual — is trackable and cancellable the same way.
...
- **Cancellable** — a "Cancel" button on any running job (`POST /api/scoring/jobs/{id}/cancel`) actually stops it via `asyncio.Task.cancel()` when the task is still alive in this process, not just a database flag flip; it also immediately clears the `409` "already running" guard, which is the more common reason to reach for it — restarting a scan against newly-ingested data without waiting for a stale or unwanted run to finish. Cancelling a job already in another terminal state (`success`/`partial`/`failed`) is rejected with `400`.
- **Self-healing on restart** — a `ScoringJob` row still `"running"` when the backend starts up cannot be a real, still-executing job: the asyncio task behind it belonged to whichever process existed before this one, and died with it. Left alone, a row like that blocks `POST /api/scoring/run`'s `409` guard **forever**, since nothing will ever move it out of `"running"` again — confirmed as a real, live incident in this dev environment (three such rows, days old, silently blocking every new scoring attempt after the backend container had been killed mid-job). The startup handler now marks any such row `"failed"` (`"Orphaned by a backend restart"`) before the app starts serving traffic.
- **Parallelized for speed** — subjects are now scored `_SCORING_CONCURRENCY` (4) at a time rather than strictly one at a time; each subject's own per-student loop is still sequential, but different subjects share no mutable state beyond the read-only `_DATA` frame (each gets its own DB session), so running several concurrently is safe and turns otherwise-idle CPU cores into real wall-clock speedup. Confirmed live: a 120-subject/~9,000-student period that previously took roughly 47 minutes end to end completed dramatically faster under concurrent scoring (helped further whenever some of those subjects already have a fresh cached prediction from an earlier run, per the cache-check above — a re-run over already-scored data was always meant to be cheap, this just makes a *fresh* run cheaper too).
- **Only one job's actual scoring work runs at a time, system-wide** (`_SCORING_SERIAL_LOCK`). **A real, reported bug**: a capstone ingestion and an attendance ingestion each auto-trigger their own `ScoringJob` completely independently, with no check for one already running — so uploading both back to back (an entirely normal, expected workflow) used to launch two jobs that ran *concurrently*, each with its own 4-way internal concurrency, doubling CPU contention for no benefit and confusing anyone watching the Risk Scoring tab wondering why two are running at once. Fixed with a lock around the actual per-subject work: a second job still gets created and shows `"running"` immediately (consistent with the manual button's own experience), it just doesn't start scoring until the first one releases the lock — and thanks to the cache-check, whatever periods it inherits that the first job already reached finish almost instantly rather than redoing real work.
...
**Test coverage**: `test_roster_caching.py` (3 tests) — a cache-hit case (fresh, live-version row; asserts `ml_predict` is monkeypatched to raise if called, proving it truly wasn't), and two staleness cases (superseded `model_version`; `predicted_at` older than the latest ingestion) each asserting the real model call happens and the fresh result wins. `test_scoring_job_control.py` (3 tests) — cancelling a genuinely in-flight job (via a monkeypatched, artificially slow `_compute_roster_rows`) actually halts its progress and clears the `409` guard for a new run; cancelling a job that isn't `"running"` is rejected; the startup handler marks an orphaned `"running"` row `"failed"`. `test_students_at_risk.py` and `test_chatbot.py` were re-run alongside the caching tests (25/25) to confirm that refactor changed nothing observable about either endpoint's response shape or scoping.

**A real incident, found and fixed, not left as a caveat**: `test_ingestion_e2e.py`'s shared `_preserve_app_state()` cleanup fixture originally deleted *every* row in `scoring_jobs` on teardown, the same blanket-delete pattern already accepted for `ingest_jobs`/`analyze_jobs`/`pending_ingests`/`upload_batches` (those only ever hold short-lived bookkeeping rows, so wiping them is harmless). `scoring_jobs` is different — it shares the table with whatever real scoring run a real admin has genuinely in progress on the same dev database — and this was confirmed live: running part of the test suite deleted two real, in-progress jobs (a capstone ingestion's and an attendance ingestion's own auto-triggered runs) out from under an actual admin simply because the suite happened to run at the same time. Fixed by snapshotting `scoring_jobs` ids on entry and diffing on exit, so only rows a test's own `confirm()` calls actually created are touched — the other four tables' existing blanket-delete behavior is untouched (out of scope here, and a much lower-risk pattern for tables that don't hold hours-long real work).
...
**117 tests across 15 files** as of this README (regenerated via `pytest --collect-only`, not hand-typed — see the file-by-file breakdown in [Project Structure](#project-structure)). A full run (`pytest tests/ -q`) passes clean. The 2 tests in `test_oauth_login.py` that assert "login rejected when no client ID is configured" will fail specifically in an environment (like this one, sometimes) that has real Google/Microsoft client IDs configured through Settings > OAuth Providers — that's the tests' own precondition no longer holding, not an application bug.

### A real incident, found while regenerating this section — not papered over

Getting to that number was not a single clean run — it surfaced a real, live data-corruption incident, documented here rather than silently fixed, in the same spirit as [Testing Practices](#testing-practices--a-real-outage-the-suite-missed) above. In order:

1. **A second full run** (after the first showed the clean 92/2 above) came back 90/4 — two new failures in `test_students_at_risk.py`. Traced to a real bug: `_preserve_app_state()` (the shared ingestion-test cleanup helper) deleted `analyze_jobs` rows before `upload_batches` rows that FK-reference them, so its own `finally`-block cleanup threw a `ForeignKeyViolationError` once real leftover `upload_batches` rows existed. **Fixed** — delete order now matches the FK dependency (`upload_batches` first).
2. **Investigating that surfaced something more serious.** `backend/app/ml/ingested_capstone.csv` — the single on-disk file this project's real dataset lives in, with no archived fallback copy ever present in this environment — had been silently overwritten with a 2-subject/951-row test fixture by an interrupted ingestion test, degrading the **live dev server itself** (`GET /api/health` dropped to `dataset.rows: 950`, versus the real 327,501). Root cause: `_isolate_ml_paths()` (the test helper whose own docstring claimed full isolation) never actually redirected `INGESTED_DATA_DIR` — the variable `main.py`'s real confirm handler writes through — so a real `confirm()` call inside a supposedly-isolated test wrote straight to the file the live server loads from. **Fixed**: `_isolate_ml_paths()` now redirects `INGESTED_DATA_DIR` (and actually reassigns `DATA_PATH`/`check_new_period.DATA_PATH`, which it had been capturing and restoring but never actually changing — dead code) to a throwaway temp directory.
3. **The dev-server footgun that let this reach production data at all**: uvicorn's `--reload` watched the entire `backend/` tree including `tests/`, so editing a test file could trigger the live server to reload and re-read whatever was currently on disk. `--reload-dir` (even an absolute path) was tried first and, confirmed live, did **not** stop this — uvicorn 0.30.6 still watched `tests/` regardless. What actually worked, verified live both directions (touching a `tests/` file → no reload; touching `app/main.py` → reload fires): `--reload-exclude "tests/*" --reload-exclude "scripts/*"`.
4. **Fixing the isolation surfaced one more real bug, not just the original one.** With `INGESTED_DATA_DIR` now genuinely isolated, `test_retrain_trigger_same_period_vs_new_period` started failing a different way: a real retrain attempt inside isolation now correctly couldn't find attendance data at the isolated path either, so `load_attendance_raw()` fell back to the archived `data/masked_attendance.csv.gz` default — which has never existed in this environment. That `FileNotFoundError` was being silently swallowed by a broad `except Exception` in `main.py`'s retrain-trigger code, surfacing only as `retrain.triggered: False` with no visible cause — the isolation bug had been accidentally masking this one too, by leaking real ambient attendance data through. **Fixed**: `_isolate_ml_paths()` now also copies the real `ingested_attendance_raw.csv` into the isolated temp directory (read-only source copy — the shared original is never touched), so an isolated retrain sees the same real data a non-isolated one would.
5. **Recovery.** No technical path existed to recover the corrupted file from within this environment — checked and ruled out: the archived `data/` fallback (never present here), Docker images (bind-mounted `app/ml/`, never baked in), the `PendingIngest.csv_bytes` row (already consumed/cleared), leftover chunked-upload `.part` files (test fixtures only), Postgres (`predictions` has no raw assessment marks), Volume Shadow Copy (needs elevation not available here), OneDrive (this project isn't under OneDrive sync). **Resolved by re-uploading the real source file** (`Capstone_data_20260729.csv` / `masked_attendance.csv`) through Data Ingestion from a separate on-disk copy the project owner still had — confirmed restored via `GET /api/health` (`dataset.rows: 327501`, `attendance.enrolments: 84422`, 100% attendance/capstone match rate) and a byte-for-byte-persistent check across a real container restart.

**All four fixes hold under the real data.** With the real dataset back and every fix in place, `test_attendance_rate_known_input` and `test_retrain_trigger_same_period_vs_new_period` both pass again — the full suite returns to the clean 92/2 stated above, this time regenerated *after* every fix in this section, not before.
...
| `test_scoring_job_control.py` | Cancelling a genuinely in-flight `ScoringJob` (via an artificially slow monkeypatched `_compute_roster_rows`) actually halts progress and clears the `409` "already running" guard; two auto-triggered jobs launched back to back (the capstone-then-attendance scenario) genuinely serialize — the second one stays at 0 subjects completed until the first releases `_SCORING_SERIAL_LOCK`; cancelling a non-running job is rejected; a `ScoringJob` left `"running"` is marked `"failed"` by the app's own startup handler, simulating exactly the orphaned-row incident found live in this dev environment |
...
| Scoring | `POST /api/scoring/run` (Head of Technology/Head of School, 202 + job id, 409 if one's already running) · `GET /api/scoring/jobs` · `GET /api/scoring/jobs/{id}` · `POST /api/scoring/jobs/{id}/cancel` — see [Background risk-prediction scoring](#background-risk-prediction-scoring--prediction-caching) |
...
- **Resolved: the `ingested_capstone.csv` corruption flagged in an earlier pass of this session turned out to be environmental, not a logic bug — tested directly, not assumed.** The file had been overwritten with a reduced (248,592-row, down from 327,501), corrupted dataset — the same class of incident documented above in [Running Tests](#running-tests). The prime suspect at the time was the fire-and-forget `_launch_scoring_job` (see [Background risk-prediction scoring](#background-risk-prediction-scoring--prediction-caching)) leaving a scoring task running past the point `_isolate_ml_paths()` tears down its isolated registry — **this was directly tested with a small-sample repro (8 real subjects, not the full 129) that launches a fire-and-forget scoring task and then yanks the isolation out from under it mid-run, on purpose**: the task kept running correctly against the real, restored registry, never touched `predictor._PACKAGE`, and a real `/api/predict` call afterward worked normally. So that race is safe. **The real cause was simpler: this environment's backend container was repeatedly observed exiting with code 137 (SIGKILL) mid-test** — Docker Desktop or the host sleeping/restarting during a long-running test run — which force-kills the process and skips any `finally`-block restore, exactly the failure mode already documented in [Running Tests](#running-tests) from an earlier, separate incident. The file was restored by exporting the still-running live server's correct in-memory data via `GET /api/explorer/export` and writing it back (minus its two derived columns), verified via a full container restart loading 327,501 rows / 129 subjects / 84,422 attendance enrolments cleanly. **Practical takeaway for whoever hits this again**: don't re-run the full-scale `test_retrain_trigger_same_period_vs_new_period` repeatedly to chase a timing issue in an environment prone to sleeping mid-test — a small real-subject subset (as done here) reproduces the same code paths in seconds and won't risk the shared file if interrupted.
