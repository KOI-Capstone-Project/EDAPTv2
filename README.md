...
│   │   ├── test_scoring_job_control.py # 3 tests — cancel + orphaned-job self-heal
...
**A caveat worth knowing, not a bug**: running the backend test suite deletes every row in `scoring_jobs` (and `ingest_jobs`/`analyze_jobs`/`pending_ingests`/`upload_batches`) as part of `test_ingestion_e2e.py`'s shared cleanup fixture — the same established, documented tradeoff those other tables already had (see that fixture's own docstring), now extended to this table too. A real scoring run in progress on a dev server will be wiped if the test suite happens to run at the same time against the same database.
