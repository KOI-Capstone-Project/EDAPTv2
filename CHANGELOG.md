...
- **The `ingested_capstone.csv` data-corruption incident flagged above is now root-caused (as environmental, not logical) and repaired.** The corrupted file was restored by exporting the still-running live server's correct in-memory dataset via `GET /api/explorer/export` (no filters), dropping its two derived columns (`PASSED`, `ATTENDANCE_RATE`) to match the raw ingest schema, and writing it back — verified via a full container restart re-loading 327,501 rows / 129 subjects / 84,422 attendance enrolments cleanly. **The suspected mechanism (a fire-and-forget scoring task still running when `_isolate_ml_paths()` tears down its isolated registry) was directly tested and ruled out**: a targeted small-sample repro launched `_launch_scoring_job` inside isolation, then restored the real registry paths and deleted the temp dir *while the task was still mid-run* — it kept running correctly against the real registry with no crash and no effect on `predictor._PACKAGE`. The actual cause was simpler and unrelated to this feature: the container was repeatedly observed exiting with code 137 (SIGKILL) mid-test-run in this environment (Docker Desktop / the host machine sleeping or restarting during a long-running test), which is exactly the "force-kill skips a `finally` restore" failure mode the README's [Running Tests](README.md#running-tests) section already documents from an earlier incident — an unlucky kill during a real file write, not a bug in the scoring trigger's logic. `test_roster_caching.py` and the manual scoring/caching paths were never implicated.
